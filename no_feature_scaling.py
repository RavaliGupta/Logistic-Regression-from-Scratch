...
def compute_error_for_graph_given_points(thetha1,thetha2,x1,x2,y):
    ...
    for i in range(1, len(x1)):
      ...
      h_thetha= 1/(1+np.e**(-((x1[i]*thetha1)+(x2[i]*thetha2))))
...
def step_gradient(thetha_current1,thetha_current2, x1,x2,y, learningRate):
    thetha_gradient1 = 0
    thetha_gradient2 = 0
    N = float(len(x1))
    max_x1=np.amax(x1)
    ...
    for i in range(1, len(x1)):   
        feature_x1= (x1[i]-mean_x1)/(max_x1-min_x1)
        feature_x2= (x2[i]-mean_x2)/(max_x2-min_x2)
        h_thetha= 1/(1+np.e**(-((x1[i]*thetha_current1)+(x2[i]*thetha_current1))))
...
def gradient_descent_runner(x1,x2,y, starting_thetha1,starting_thetha2, learning_rate, num_iterations):
    thetha1 = starting_thetha1
    thetha2 = starting_thetha2
    for i in range(num_iterations):
        [new_thetha1,new_thetha2] = step_gradient(thetha1,thetha2, x1,x2,y, learning_rate)    
    #print("",)
    return [new_thetha1,new_thetha2]
...
def sigmoid(thetha1,thetha2,x1,x2):
    ...
    #y=np.exp(b*feature_x)/(1+np.exp(b*feature_x))
        a.append(1/(1+np.e**(-((x1[i]*thetha1)+(x2[i]*thetha2)))))
...
def run():
   ...
   # plt.show()
    ...
    print ("Running...")  
    [thetha1,thetha2] = gradient_descent_runner(x1,x2,y, initial_thetha1,initial_thetha2, learning_rate, num_iterations)    
    max_x=np.amax(x1_test)
    min_x=np.amin(x1_test)
    mean_x=np.mean(x1_test)
    for item in range(len(x1_test)):
        feature_x= (item-mean_x)/(max_x-min_x)
        x_features.append(x1_test[item])
   ...
   # print("value of feature x after appending array is:::",x_array)
    ...
    print("false positive is:::",df_confusion[1][0])
    print("false neg is:::",df_confusion[0][1])
